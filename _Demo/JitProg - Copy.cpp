...
JitProg::JitProg():oEE(0),oBuff(0), dUpdate(0), bHaveUpdate(false){
};
...
oModule = std::unique_ptr<llvm::Module>( new llvm::Module("module", Context ) );
    llvm::EngineBuilder engine_builder(std::move(oModule));
...
/*
        ...
        std::unique_ptr<llvm::Module> MyModule(reinterpret_cast<llvm::Module*>(oModule)) ;
     //   MyModule = std::move(reinterpret_cast<llvm::Module*>(oModule));
        llvm::EngineBuilder engine_builder(std::move(MyModule));
        ...
        oEE->reset(engine_builder.create());
        oEE->finalizeObject();
*/
...
bool JitProg::fPrint(){
    if(oModule){
        LLVMDumpModule((LLVMModuleRef*)*oModule.get());
    }
}
